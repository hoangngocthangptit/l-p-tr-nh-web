 ...
 * @author Duc Huy
 */
@WebServlet(name = "AddNewUserController", urlPatterns = {"/add"})
public class AddNewUserController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String role = request.getParameter("System Role");
        ...
        if (count > 0) {
            request.setAttribute("fail", "Add user fail ! Try again");
            userDAO.close();
            request.getRequestDispatcher("addnewuser.jsp").forward(request, response);
        } else {
            userDAO.AddNewUser(username, name, email, mobile, gender, role, new Hash().HashPassword(pass));
            Email mail = new Email();
            mail.MailSendPassword(email , "pass: " + pass + "user:" + "" + username );
            request.setAttribute("success", "Added Sucessfully");
            request.getRequestDispatcher("addnewuser.jsp").forward(request, response);
        }             
    }
